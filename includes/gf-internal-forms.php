		...
		'fields' => array(
			...
			array(
				...
				'choices' => array(
					...
					array('text' => 'Minor', 'value' => 'minor'),
					array('text' => 'Severe', 'value' => 'severe'),
				),
	...
	$form = array(
		...
		'fields' => array(
			...
			array(
				...
				'enableChoiceValue' => true,
				'choices' => array(
					array('text' => 'Cleared', 'value' => 'cleared'),
					array('text' => 'Cleared with restrictions', 'value' => 'cleared_restrictions'),
					array('text' => 'Not Cleared', 'value' => 'not_cleared'),
				),
			...
			array(
				...
				'enableChoiceValue' => true,
				'choices' => array(
					array('text' => 'Respiratory', 'value' => 'respiratory'),
					array('text' => 'Musculoskeletal', 'value' => 'musculoskeletal'),
					array('text' => 'Cardiovascular', 'value' => 'cardio'),
					array('text' => 'Contagious Risk', 'value' => 'contagious'),
					array('text' => 'Medication Effects', 'value' => 'medication_effects'),
				),
...
function jrj_seed_internal_demo(array $ids): void
{
	...
	if (!empty($ids['skills'])) {
		$rows = array(
			array('email' => 'alfa@example.com', 'assessor' => 'Maria Lopez', 'scores' => [4, 4, 5, 4, 4, 4], 'flag' => 'none'),
			array('email' => 'bravo@example.com', 'assessor' => 'James Carter', 'scores' => [3, 3, 3, 3, 3, 3], 'flag' => 'minor'),
		);
		foreach ($rows as $r) {
	...
	if (!empty($ids['physical'])) {
		$rows = array(
			array('email' => 'charlie@example.com', 'assessor' => 'Ava Johnson', 'scores' => [5, 4, 5, 4, 5, 5]),
			array('email' => 'delta@example.com', 'assessor' => 'Olivia Brown', 'scores' => [3, 3, 4, 3, 3, 4]),
		);
		foreach ($rows as $r) {
			$raw = array_sum($r['scores']);
			$norm = round(($raw / 30) * 100, 0);
	...
	if (!empty($ids['medical'])) {
		$rows = array(
			array('email' => 'alfa@example.com', 'pract' => 'Dr. Green', 'clear' => 'cleared', 'raw' => null, 'flags' => ['respiratory']),
			array('email' => 'bravo@example.com', 'pract' => 'Dr. Brown', 'clear' => 'cleared_restrictions', 'raw' => 85, 'flags' => ['musculoskeletal']),
		);
		foreach ($rows as $r) {
			$norm = is_numeric($r['raw']) ? max(0, min(100, (float) $r['raw'])) : ($r['clear'] === 'cleared' ? 100 : ($r['clear'] === 'cleared_restrictions' ? 70 : 0));
			$cb = [];
			$choices = ['respiratory', 'musculoskeletal', 'cardio', 'contagious', 'medication_effects'];
