 ...
 *  5 endurance(0-5), 6 strength(0-5), 7 flexibility(0-5),
 *  8 injuries (checkbox), 9 notes, 10 attachments,
 *  11 raw_total (calc), 12 normalized (calc 0–100)
 */
function jrj_create_physical_form(): int {
	...
	$form  = array(
		...
		'fields'        => array(
			...
			// Six criteria (0–5) — all with inputName used by handler
			array(
				'id'        => 5,
				'label'     => 'Strength (0–5)',
				'type'      => 'number',
			...
			// Optional integrity flag used by handler
			array(
				'id'                => 11,
			...
			// Calc fields only for UI (server-side calculate)
			array(
				'id'          => 12,
				'label'       => 'Physical Raw Total',
				'type'        => 'number',
				'calculation' => true,
				'formula'     => '{5}+{6}+{7}+{8}+{9}+{10}',
				'readOnly'    => true,
			),
			array(
				'id'          => 13,
				...
				'calculation' => true,
				// client says max = 20
				'formula'     => 'round( ({Physical Raw Total:12}/20)*100, 0 )',
				'readOnly'    => true,
			),
		),
		'confirmations' => array(
 ...
 * IDs:
 *  1 applicant_id, 2 applicant_email, 3 date, 4 assessor,
 *  5–10 six criteria (0–5), 11 raw_total (calc), 12 integrity_flag (radio),
 *  13 notes, 14 attachments, 15 normalized (calc)
 */
 ...
 * IDs:
 *  1 applicant_id, 2 applicant_email, 3 date, 4 practitioner,
 *  5 clearance (radio), 6 flags (checkbox), 7 notes, 8 attachments,
 *  9 medical_raw (optional), 10 medical_norm (number; we fill via hook/seed)
 */
...
/**
 * Optional: seed a few demo entries when JRJ_DEV_MODE is true.
 */
