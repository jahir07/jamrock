...
(function ($) {
  ...
  $(document).on("click", '.feedback-form button[type="button"]', function (e) {
    ...
    Object.entries(fields).forEach(([key, value]) => {
      const $input = $form.find(`.input${key.charAt(0).toUpperCase() + key.slice(1)}`);
    ...
    $.ajax({
      ...
      success: function (response) {
        const messageClass = response.status === "success" ? "text-success" : "text-danger";
        $form.closest(".current-form").parent().html(`<h3 class="${messageClass}">${response.message}</h3>`);
      },
      error: function () {
        $form.find(".alert-msg").text("Something went wrong. Please try again.");
      }
    });
  });
...
/* harmony import */ var _includes_Feedback_js__WEBPACK_IMPORTED_MODULE_2__ = __webpack_require__(/*! ./includes/Feedback.js */ "./includes/Feedback.js");
/* harmony import */ var _includes_Feedback_js__WEBPACK_IMPORTED_MODULE_2___default = /*#__PURE__*/__webpack_require__.n(_includes_Feedback_js__WEBPACK_IMPORTED_MODULE_2__);
